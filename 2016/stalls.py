	...
	for halving in range(nearest_power_of_2_below):
		...
		for section_size, count in sections:
			...
			if odd(section_size):
				...
				new_sections[int((section_size - 1) / 2)] += count * 2

			else:
				assert even(section_size)

				new_sections[int((section_size - 2) / 2)] += count
				new_sections[int((section_size) / 2)] += count

		sections = list(new_sections.items())
		print(sections)
		new_sections.clear()
